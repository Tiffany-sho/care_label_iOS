 ...
 *
 * 閾値は**出荷するこの実装で**合成データから引き直した（`tools/sweep_shade.cjs`）。
 * 最初の 0.145 は測定側の Python 実装で出した値だったが、そちらは
 * フラットフィールド補正をしない素の大津で二値化していて、**同じ量を
 * 測っていなかった**（実写の負例1件で 0.121 と 0.144 に食い違った）。
 *
 * 合成314件（自然乾燥のみ、劣化度別の日陰あり/なしの min/中央/max）:
 *   s0  0.179/0.209/0.233 対 -0.040/-0.005/0.010   分離
 *   s1  0.171/0.214/0.290 対 -0.065/-0.000/0.037   分離
 *   s2  0.112/0.197/0.300 対 -0.088/-0.002/0.063   分離
 *   s3 以上は重なる（他の属性も総崩れになる劣化度）
 * 閾値ごとの誤り（誤検出 / 見落とし）:
 *   0.105 -> 1/7   0.120 -> 1/9   0.145 -> 0/14   0.160 -> 0/21   0.200 -> 0/65
 * 実写20枚の通しは 0.105〜0.145 で同じ（一致111）、0.16 で 110 に落ちる。
 * **両方で平らな 0.10〜0.12 の中央を採る。**
 *
 * 見落とし側に倒すか誤検出側に倒すかは、誤りの意味で決めた。
 * 日陰を見落とす＝「日陰でつり干し」を「つり干し」と言う＝日なたに干させる。
 * 逆に日陰と言いすぎるのは、日陰に干させるだけで衣類を傷めない。
 * **見落としのほうが危険**なので、誤検出0を狙って閾値を上げるのではなく、
 * 誤りの総数が最小になるあたりを採る。
 *
 * ⚠️ **実写にはまだ「日陰でない自然乾燥」が1件しか無く、その1件を外している。**
 * 実写での 15/16（93.8%）は「日陰と答えておけばこうなる」状態と区別がつかない。
 * この数字を検出器の実力として読まないこと。閾値を下げた判断が正しかったかは、
 * 平干し系の日陰なし（420/410/415/430/435）の写真が入るまで検証できない。
 * 写真が届いたら、`tools/diag_shade.cjs`（この実装）と
 * `tools/probe_shade.py`（素の大津での独立実装）を両方回し、
 * **値ではなく分離の構造**が一致するかで突き合わせること。
 *
 * ⚠️ 極性はタグ全体で決めた値を使うこと。記号1個で決めると、枠の縁に輪郭が
 * 触れている写真で反転し、符号ごとひっくり返る（実測で -0.269 と +0.195）。
 */
export const SHADE_MIN_SCORE = 0.11;
